...
class UploadFile {//类定义开始
    ...
    public $thumb   =  true;
    // 图库类包路径
    public $imageClassPath = CLASS_PATH;
    // 缩略图最大宽度
    public $thumbMaxWidth="900";
    // 缩略图最大高度
    public $thumbMaxHeight="700";
    // 缩略图前缀
    public $thumbPrefix   =  ',';
    public $thumbSuffix  =  ',';
    // 缩略图保存路径
    public $thumbPath = 'Uploads/';
    // 缩略图文件名
    public $thumbFile		=	'';
    // 是否移除原图
    ...
    // 上传文件Hash规则函数名
    // 例如可以是 md5_fithumble sha1_file 等
        ...
        */
        ...
        if($this->thumb && in_array(strtolower($file['extension']),array('gif','jpg','jpeg','bmp','png'))) {
            ...
            if(false !== $image) {
                ...
                // 生成图像缩略图
                require $this->imageClassPath.'/Image.class.php';
    ...
    public function upload($savePath ='') {
        ...
        foreach($files as $key => $file) {
            ...
            if(!empty($file['name'])) {
                ...
                $function=$this->hashType;
		        $file['savename']   = $function($file['tmp_name']).".".$file['extension'];//采用hash验证的方法,自定义 
                // 自动检查附件
                if($this->autoCheck) {
                    if(!$this->check($file))
                        return false;
